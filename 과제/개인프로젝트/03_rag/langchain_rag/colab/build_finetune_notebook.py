"""RAG-instruction 재파인튜닝 Colab 노트북 생성기.

내 모델(polyglot58-chatbot) 위에 LoRA 로 '문서 근거로 답하기 + 없으면 거부하기'를
학습시킨다. 학습셋은 docs 의 사실에서 만든 (문맥+질문->근거답변) + 거부 예시.
"""
import json
import os

ROOT = "/Users/minwoo/7주차/langchain_rag"
OUT = os.path.join(ROOT, "colab", "otto_rag_finetune_colab.ipynb")

REFUSE = "제공된 문서에서 답을 찾을 수 없습니다."

# --- 문맥 스니펫 (docs 의 실제 단락, 검색결과 형식과 동일하게 [출처:] 접두) ---
C = {
...
# (context_key 또는 결합, 질문, 답변)
grounded = [
    ("adapterz", "어댑터즈 월 구독료는 얼마인가요?", "월 구독료는 1만 9천 원입니다."),
    ("adapterz", "어댑터즈 구독료 얼마야?", "월 1만 9천 원입니다."),
    ("adapterz", "어댑터즈는 어떤 서비스야?", "현업 개발자가 집필한 실무 중심 개발 교재를 구독형으로 제공하는 서비스입니다."),
    ("adapterz", "어댑터즈 교재는 얼마나 자주 업데이트돼?", "매주 새로 업데이트됩니다."),
    ("adapterz", "어댑터즈 코드 예제는 어디서 받아?", "깃허브 저장소로 함께 제공됩니다."),
    ("company", "스타트업코드는 언제 설립됐어?", "2021년에 설립되었습니다."),
    ("company", "누적 수강생은 몇 명이야?", "1만 2천 명을 넘었습니다."),
    ("company", "스타트업코드는 어떤 회사야?", "개발자를 위한 교육과 교재를 제공하는 회사입니다."),
    ("company", "스타트업코드 주력 서비스가 뭐야?", "개발 교재 플랫폼 어댑터즈와 온라인 코딩 부트캠프입니다."),
    ("bootcamp", "부트캠프는 몇 주 과정이야?", "16주 과정입니다."),
    ("bootcamp", "부트캠프 트랙은 어떤 게 있어?", "백엔드 트랙과 AI 트랙 두 가지가 있습니다."),
    ("bootcamp", "백엔드 트랙은 뭘 배워?", "파이썬과 FastAPI를 배웁니다."),
    ("bootcamp", "AI 트랙은 뭘 다뤄?", "딥러닝과 LLM 애플리케이션 개발을 다룹니다."),
    ("bootcamp", "수료 조건이 뭐야?", "최종 프로젝트를 제출하고 코드 리뷰를 통과해야 합니다."),
    ("refund", "부트캠프 수강료는 얼마야?", "350만 원이며 분할 납부가 가능합니다."),
    ("refund", "수강료 분할 납부 돼?", "네, 분할 납부가 가능합니다."),
    ("refund", "환불은 언제까지 신청하면 전액 환불돼?", "수강 시작 후 14일 이내에 신청하면 전액 환불됩니다."),
    ("refund", "14일 지나면 환불 어떻게 돼?", "진행한 주차를 제외한 잔여 금액의 일부만 환불됩니다."),
    ("cert", "수료하면 뭘 받아?", "수료증이 발급됩니다."),
    ("cert", "취업 지원은 어떤 게 있어?", "이력서 첨삭과 모의 면접을 제공하고 협력 기업에 추천서를 보내줍니다."),
    ("contact", "문의는 어디로 해?", "이메일(support@startupcode.kr) 또는 카카오톡 채널로 문의할 수 있습니다."),
    ("contact", "상담 가능 시간은?", "평일 오전 10시부터 오후 6시까지입니다."),
    ("contact", "주말에도 문의 답변 받을 수 있어?", "주말과 공휴일에는 답변이 다음 영업일로 넘어갑니다."),
    ("aigoal", "AI 트랙 수료하면 뭘 만들 수 있어?", "이미지 분류 모델과 RAG 기반 챗봇을 만들 수 있는 수준을 목표로 합니다."),
    ("aigoal", "AI 트랙은 무엇부터 다뤄?", "딥러닝의 기초부터 LLM 애플리케이션 개발까지 다룹니다."),
    ("weeks", "1주차엔 뭘 배워?", "파이썬 복습과 넘파이를 다룹니다."),
    ("weeks", "2주차부터 4주차까지는 뭘 배워?", "신경망과 CNN을 배우고 ResNet과 VGG16 같은 사전학습 모델로 전이학습을 실습합니다."),
    ("weeks", "5주차엔 뭘 해?", "자연어 처리와 챗봇을 만들고 RAG 아키텍처를 적용합니다."),
    ("weeks", "전이학습은 어떤 모델로 실습해?", "ResNet과 VGG16 같은 사전학습 모델로 실습합니다."),
    ("ragprac", "RAG 실습에서 임베딩은 뭘로 해?", "sentence-transformers로 임베딩합니다."),
    ("ragprac", "임베딩한 건 어디에 저장해?", "ChromaDB에 저장합니다."),
    ("ragprac", "관련 문서는 어떻게 검색해?", "코사인 유사도로 검색합니다."),
    ("eval", "최종 프로젝트는 뭘로 평가해?", "동작 여부, 코드 구조, 발표로 평가됩니다."),
    ("eval", "개념만 알면 높은 평가 받아?", "단순히 개념을 아는 것보다 실제로 동작하는 애플리케이션을 만드는 것이 더 높게 평가됩니다."),
]

# 멀티 문맥(방해 문서 포함) — 관련 문서를 골라 답하는 능력 학습
multi = [
    (["refund", "adapterz"], "부트캠프 수강료 얼마야?", "350만 원이며 분할 납부가 가능합니다."),
    (["adapterz", "bootcamp"], "어댑터즈 구독료는?", "월 1만 9천 원입니다."),
    (["company", "contact"], "스타트업코드 설립 연도는?", "2021년입니다."),
    (["weeks", "refund"], "5주차에 뭐 배워?", "자연어 처리와 챗봇을 만들고 RAG 아키텍처를 적용합니다."),
    (["cert", "ragprac"], "수료하면 수료증 주나요?", "네, 모든 과정을 수료하면 수료증이 발급됩니다."),
]

# 거부 예시 — 문맥은 있지만 답이 없을 때
refusals = [
    ("adapterz", "부트캠프 수강료 얼마야?"),
    ("refund", "어댑터즈 구독료 얼마야?"),
    ("bootcamp", "환불 정책 알려줘"),
    ("contact", "수강료 얼마야?"),
    ("weeks", "수료증 주나요?"),
...
# 최종 학습 데이터 빌드
train_rows = []
for k, q, a in grounded:
    train_rows.append({"context": C[k], "question": q, "answer": a})
for keys, q, a in multi:
    train_rows.append({"context": "\n\n".join(C[k] for k in keys), "question": q, "answer": a})
for k, q in refusals:
    train_rows.append({"context": C[k], "question": q, "answer": REFUSE})
...
cells.append(md(
    ...
    "\n",
    f"학습셋 {len(train_rows)}개 (근거답변 {len(grounded)+len(multi)} + 거부 {len(refusals)}) — 내 docs 사실 기반.\n",
    "\n",
...
cells.append(md("## 4. 학습 데이터 (내 문서 기반)"))
cells.append(code(
    "train_rows = " + json.dumps(train_rows, ensure_ascii=False, indent=1) + "\n"
...
cells.append(md("## 6. 토크나이즈 (프롬프트는 손실 제외, 답변만 학습)"))
cells.append(code(
...
cells.append(md("## 8. 학습 후 동작 테스트 (근거답변 + 거부)"))
cells.append(code(
    "import torch\n"
    "model.eval()\n"
    "\n"
...
print("생성:", OUT, "| 셀", len(cells), "| 학습예시", len(train_rows))
